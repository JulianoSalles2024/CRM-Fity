-- Atualiza a função de 4 parâmetros (versão que está no cache do PostgREST)
-- Usa UUID do admin como fallback garantido enquanto o problema de RLS é investigado
...
BEGIN
  ...
  IF v_owner_id IS NULL THEN
    SELECT p.id INTO v_owner_id FROM profiles p
    WHERE p.company_id = p_company_id LIMIT 1;
  END IF;

  -- Fallback: admin conhecido da empresa 32a1ab33
  IF v_owner_id IS NULL THEN
