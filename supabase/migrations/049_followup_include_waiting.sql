-- Migration 049: Allow get_pending_followups to process 'waiting' conversations
-- Previously only 'in_progress' conversations triggered follow-up.
-- Now 'waiting' conversations are also included so the sequence starts
-- without requiring an agent to manually assume the conversation first.
...
AS $$
  ...
  WHERE
    -- Inclui 'waiting' além de 'in_progress'
