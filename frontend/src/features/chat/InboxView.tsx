...
// Maps a stat's text color class to a card container style
...
// Calculates days since the lead's last activity
const daysSinceActivity = (lead: Lead): number => {
    const lastDate = lead.lastActivityTimestamp
        ? new Date(lead.lastActivityTimestamp)
...
const InboxView: React.FC<InboxViewProps> = ({ tasks, leads, users = [], onNavigate, onOpenLead, onUpdateTaskStatus, mode = 'standard', currentUserRole = 'seller', userId }) => {
    ...
    // Focus mode: current task by index
    const currentFocusTask = allPendingTasks[currentFocusIndex] ?? null;
    const currentFocusLead = useMemo(() => {
        if (!currentFocusTask?.leadId) return null;
        return leads.find(l => l.id === currentFocusTask.leadId) ?? null;
    }, [currentFocusTask, leads]);

    // Reset focus index when entering focus mode
    useEffect(() => {
        if (viewMode === 'focus') setCurrentFocusIndex(0);
    }, [viewMode]);
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.code === 'Space' && tag !== 'INPUT' && tag !== 'TEXTAREA') {
                e.preventDefault();
                if (currentFocusLead) onOpenLead?.(currentFocusLead);
            }
        };
        window.addEventListener('keydown', handleKeyDown);
        return () => window.removeEventListener('keydown', handleKeyDown);
    }, [viewMode, currentFocusLead, onOpenLead]);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {stats.map((stat, i) => (
                            <div
                                ...
                                className={`p-5 rounded-xl border backdrop-blur-sm flex flex-col justify-between min-h-[7rem] py-5 ${stat.onClick ? 'cursor-pointer transition-all hover:brightness-110' : ''} ${getContainerClass(stat.color)}`}
                            >
                                    ...
                                    <div key={lead.id} className="flex items-center justify-between group">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500">{lead.name} não interage há {daysSinceActivity(lead)} dias</p>
                                            </div>
                                        </div>
                    ...
                    {currentFocusTask ? (
                        ...
                        <div className="w-full max-w-xl bg-slate-900/60 border border-slate-700/50 rounded-2xl p-8 backdrop-blur-sm space-y-6">
                            <div className="flex items-start gap-4">
                                <div className={`mt-1 p-2 rounded-lg ${overdueTasks.includes(currentFocusTask) ? 'bg-red-500/10' : 'bg-sky-500/10'}`}>
                                    {overdueTasks.includes(currentFocusTask)
                                        ? <AlertCircle className="w-5 h-5 text-red-400" />
                                        : <Calendar className="w-5 h-5 text-sky-400" />
                                    }
                                </div>
                                <div className="flex-1">
                                    <span className={`text-xs font-bold uppercase tracking-widest ${overdueTasks.includes(currentFocusTask) ? 'text-red-400' : 'text-sky-400'}`}>
                                        {overdueTasks.includes(currentFocusTask) ? 'Atrasada' : 'Para hoje'}
                                    </span>
                                    <p className="text-xl font-semibold text-white mt-1">{currentFocusTask.title}</p>
                                    {currentFocusTask.description && (
                                        <p className="text-sm text-slate-400 mt-2">{currentFocusTask.description}</p>
                                    )}
                                    <p className="text-xs text-slate-500 mt-3">
                                        Vence em: {new Date(currentFocusTask.dueDate).toLocaleDateString('pt-BR')}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3 pt-2 border-t border-slate-700/40">
                                ...
                                {currentFocusLead && (
                                    <div className="flex items-center gap-2">
                                        <button
                                            onClick={() => onOpenLead?.(currentFocusLead)}
                                        ...
                                        <span className="text-[10px] px-2 py-1 rounded border border-yellow-400/30 bg-yellow-400/8 text-yellow-400/50 font-bold tracking-wider whitespace-nowrap">
                                            ⎵ SPACE
                                        </span>
                                    </div>
                                )}
                        ...
                        {/* Task navigation */}
                        {allPendingTasks.length > 1 && (
                            <div className="flex items-center gap-4">
                                <button
                                    onClick={() => setCurrentFocusIndex(i => Math.max(0, i - 1))}
                                    disabled={currentFocusIndex === 0}
                                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-700 text-slate-400 hover:text-white hover:border-slate-500 disabled:opacity-30 disabled:cursor-not-allowed transition-all text-sm"
                                >
                                    <ChevronLeft className="w-4 h-4" /> Anterior
                                </button>
                                <span className="text-xs text-slate-600 tabular-nums">
                                    {currentFocusIndex + 1} / {allPendingTasks.length}
                                </span>
                                <button
                                    onClick={() => setCurrentFocusIndex(i => Math.min(allPendingTasks.length - 1, i + 1))}
                                    disabled={currentFocusIndex === allPendingTasks.length - 1}
                                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-700 text-slate-400 hover:text-white hover:border-slate-500 disabled:opacity-30 disabled:cursor-not-allowed transition-all text-sm"
                                >
...
const TaskRow: React.FC<{ task: Task; badge: 'atrasada' | 'hoje'; onComplete?: (taskId: string) => void }> = ({ task, badge, onComplete }) => {
    ...
    const formattedDate = new Date(task.dueDate).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
