...
export function mapLeadFromDb(row: Record<string, unknown>): Lead {
  return {
    ...
    lastActivityTimestamp: (row.last_activity_timestamp as string) ?? null,
    lastActivityType: (row.last_activity_type as string) ?? null,
...
export function mapLeadToDb(lead: Partial<Lead>): Record<string, unknown> {
  return omitUndefined({
    ...
    value: lead.value,
    source: lead.source,
    status: 'status' in lead ? (lead.status ?? null) : undefined,
    probability: lead.probability,
    tags: lead.tags !== undefined ? lead.tags : undefined,
    last_activity: lead.lastActivity,
    last_activity_timestamp: lead.lastActivityTimestamp,
    last_activity_type: 'lastActivityType' in lead ? (lead.lastActivityType ?? null) : undefined,
