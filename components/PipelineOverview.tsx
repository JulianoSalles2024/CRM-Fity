...
const PipelineOverview: React.FC<PipelineOverviewProps> = ({ columns, leads, onNavigate }) => {
    ...
    return (
        <div className="bg-white dark:bg-zinc-800 p-5 rounded-lg border border-zinc-200 dark:border-zinc-700 h-full flex flex-col transition-all duration-200 ease-in-out hover:bg-gray-50 dark:hover:bg-zinc-700/50 hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="flex-1 space-y-2">
                {overviewData.map(stage => (
                    <div key={stage.id} className="p-3 bg-gray-100 dark:bg-zinc-900/50 rounded-md flex justify-between items-center transition-colors hover:bg-gray-200 dark:hover:bg-zinc-700/50">
                        <div>
