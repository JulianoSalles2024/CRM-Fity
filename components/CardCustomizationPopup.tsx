...
const ToggleSwitch: React.FC<{ checked: boolean; onChange: (checked: boolean) => void }> = ({ checked, onChange }) => (
    <button onClick={() => onChange(!checked)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${checked ? 'bg-violet-600' : 'bg-slate-600'}`}>
...
const CardCustomizationPopup: React.FC<CardCustomizationPopupProps> = ({ settings, onUpdate, onClose }) => {
    ...
    const settingOptions: { key: keyof CardDisplaySettings; label: string }[] = [
        ...
        { key: 'showDueDate', label: 'Mostrar Data de Vencimento' },
        ...
        { key: 'showStage', label: 'Mostrar Estágio' },
    ];
